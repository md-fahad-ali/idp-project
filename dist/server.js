...
const ai_quiz_generator_simple_1 = require("./ai-quiz-generator.simple");
...
nextApp
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('identify', async ({ userId }) => {
            ...
            try {
                ...
                await User_1.default.findByIdAndUpdate(userId, { lastActive: currentTime }, { new: true });
            }
            catch (error) {
        ...
        socket.on('create_challenge', async (data) => {
            try {
                ...
                if (!challenger || !challenged || !course) {
                    ...
                    const generatedQuestions = await (0, ai_quiz_generator_simple_1.generateQuestionsForCourse)(mockCourseContent);
        ...
        socket.on('create_challenge_room', async (data) => {
            ...
            try {
                ...
                const existingChallenge = Object.values(challengeRooms).find(room => room.challengerId === challengerId &&
            ...
            if (!room.questions || room.questions.length === 0) {
                ...
                Course_1.default.findById(room.courseId)
                    ...
                    try {
                        ...
                        const generatedQuestions = await (0, ai_quiz_generator_simple_1.generateQuestionsForCourse)(courseContent);
                        if (generatedQuestions.length === 0) {
        ...
        socket.on('submit_answer', ({ roomId, userId, questionId, answer, timeSpent, answerLetter }) => {
            try {
                ...
                if (!isCorrect && typeof currentQuestion.correctAnswer === 'string') {
                    const correctAnswerText = currentQuestion.options.find((option, index) => String.fromCharCode(65 + index) === currentQuestion.correctAnswer);
    ...
    const endChallenge = async (roomId) => {
        ...
        let winnerId;
        let winnerName;
        let tiebreakerId = null; // ID of the user who won by completing faster in a tie
        let badgeAwarded = null; // Track which badge was awarded
        let isTie = false;
        if (challengerScore > challengedScore) {
            winnerId = room.challengerId;
            winnerName = room.challengerName;
        ...
        try {
            ...
            const winnerUser = await User_1.default.findById(winnerId);
            if (winnerUser) {
            ...
            const highestScoreUser = await User_1.default.findOne({
            ...
            if (highestScoreUser) {
                ...
                const currentCourseHighScore = await TestResult_1.default.findOne({
                    courseId: room.courseId
