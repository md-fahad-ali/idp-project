...
router.get("/stats", authenticateJWT, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const totalAdminCourses = await Course.countDocuments({ user: userId });
    const adminCourses = await Course.find({ user: userId })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    const userProgressData = await UserProgress.find().populate('user').populate({
...
router.get("/get", authenticateJWT, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const adminTotalCourses = await Course.countDocuments(adminQuery);
  ...
  try {
    ...
    try {
      ...
      if (mongoose.Types.ObjectId.isValid(courseId)) {
        ...
        course = await Course.findById(courseId, projection)
          .populate("user", "firstName lastName email");
      }
    } catch (err) {
    ...
    if (!course) {
      try {
        ...
        course = await Course.findOne({ title: courseId }, projection)
          .populate("user", "firstName lastName email");
      } catch (err) {
...
router.get("/lesson/:courseId/:lessonIndex", authenticateJWT, async (req, res): Promise<void> => {
...
router.get("/get-by-slug/:slug", authenticateJWT, async (req, res): Promise<void> => {
